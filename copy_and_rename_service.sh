...
# --- НАСТРОЙКИ ---
# Укажите полный путь к директории ПОСЛЕ ПЕРВОГО ЭТАПА (где все "notes")
SOURCE_DIR="/home/jean/second_brain/services/notes"
# Укажите полный путь к НОВОЙ директории, куда будет скопирован и изменен сервис
DEST_DIR="/home/jean/second_brain/services/calendar"
# -----------------
...
echo "  - 'notes' -> 'calendar' (в контенте)"
echo "  - 'Notes' -> 'Calendar' (в контенте)"
echo "  - имена файлов/директорий 'notes...' -> 'calendar...'"
echo ""
...
find "$DEST_DIR" -depth -name "*${rename_pattern}*" -print0 | while IFS= read -r -d $'\0' old_path; do
    ...
    # ВНИМАНИЕ: Это заменит 'notes' на 'calendar' даже если 'notes' является частью другого слова,
    # например 'anotes_file' станет 'acalendar_file'. Если такое поведение нежелательно,
