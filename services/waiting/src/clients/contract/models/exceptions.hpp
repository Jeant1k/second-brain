...
public:
    HttpClientException(
        ...
        std::optional<waiting::handlers::Error>&& error_body = std::nullopt
    )
    ...
    const std::optional<waiting::handlers::Error>& GetErrorBody() const { return error_body_; }

private:
    userver::clients::http::Status status_code_;
    std::optional<waiting::handlers::Error> error_body_;
};

class BadRequestError : public HttpClientException {
public:
    explicit BadRequestError(std::optional<waiting::handlers::Error>&& error_body = std::nullopt)
...
public:
    explicit NotFoundError(std::optional<waiting::handlers::Error>&& error_body = std::nullopt)
...
public:
    explicit InternalServerError(std::optional<waiting::handlers::Error>&& error_body = std::nullopt)
        : HttpClientException(
