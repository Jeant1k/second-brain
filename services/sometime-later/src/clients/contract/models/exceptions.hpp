...
public:
    HttpClientException(
        ...
        std::optional<current_actions::handlers::Error>&& error_body = std::nullopt
    )
    ...
    const std::optional<current_actions::handlers::Error>& GetErrorBody() const { return error_body_; }

private:
    userver::clients::http::Status status_code_;
    std::optional<current_actions::handlers::Error> error_body_;
};

class BadRequestError : public HttpClientException {
public:
    explicit BadRequestError(std::optional<current_actions::handlers::Error>&& error_body = std::nullopt)
...
public:
    explicit NotFoundError(std::optional<current_actions::handlers::Error>&& error_body = std::nullopt)
...
public:
    explicit InternalServerError(std::optional<current_actions::handlers::Error>&& error_body = std::nullopt)
        : HttpClientException(
