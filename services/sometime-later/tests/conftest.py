...
def initial_data_path(service_source_dir):
    ...
    return [
        service_source_dir / 'tests/test_sometime_later/static/test_create_task/testcases',
        service_source_dir / 'tests/test_sometime_later/static/test_complete_task/testcases',
        service_source_dir / 'tests/test_sometime_later/static/test_reactivate_task/testcases',
    ]
...
def pgsql_local(service_source_dir, pgsql_local_create):
    ...
    databases = discover.find_schemas(
        'sometime-later',  # service name that goes to the DB connection
...
# @pytest.fixture(scope='session')
# def service_config_yaml_override(mockserver_info):
#     """
#     Переопределяет переменные конфигурации для тестового окружения.
#     В данном случае, мы направляем current-actions-client на mockserver.
#     """
#     return {
#         'current-actions-client.base-url': mockserver_info.base_url,
#     }
